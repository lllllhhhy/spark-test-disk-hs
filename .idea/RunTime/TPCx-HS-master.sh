...
sudo mkdir -p /hdfs-fast
# 读写
sudo mkdir -p /hdfs-fast/input
# sort排完序 写到这里
sudo mkdir -p /hdfs-fast/output
# 数据验证放到这里
sudo mkdir -p /hdfs-fast/validate
###############################################################################
...
# 如果有数据，先删除之
sudo rm /hdfs-fast/input/*
sudo rm /hdfs-fast/output/*
sudo rm /hdfs-fast/validate/*
...
if [ "$FRAMEWORK" = "MapReduce" ]; then
    (time hadoop jar $HSSORT_JAR HSGen -Dmapreduce.job.maps=$NUM_MAPS -Dmapreduce.job.reduces=$NUM_REDUCERS -Dmapred.map.tasks=$NUM_MAPS -Dmapred.reduce.tasks=$NUM_REDUCERS $hssize /hdfs-fast/input) 2> >(tee ./logs/HSgen-time-run$i.txt) 
else
    (time spark-submit --class HSGen --deploy-mode ${SPARK_DEPLOY_MODE} --master ${SPARK_MASTER_URL} --conf "spark.driver.memory=${SPARK_DRIVER_MEMORY}" --conf "spark.executor.memory=${SPARK_EXECUTOR_MEMORY}" --conf "spark.executor.cores=${SPARK_EXECUTOR_CORES}" --conf "spark.executor.instances=${SPARK_EXECUTOR_INSTANCES}" ${HSSORT_JAR} ${hssize} /hdfs-fast/input) 2>&1 | (tee ./logs/HSgen-time-run${i}.txt)
fi
result=$?
...
echo "" | tee -a ./TPCx-HS-result-"$prefix".log
./HSDataCheck.sh /hdfs-fast/input/  | tee -a ./TPCx-HS-result-"$prefix".log
echo "" | tee -a ./TPCx-HS-result-"$prefix".log
...
if [ "$FRAMEWORK" = "MapReduce" ]; then
    (time hadoop jar $HSSORT_JAR HSSort -Dmapreduce.job.maps=$NUM_MAPS -Dmapreduce.job.reduces=$NUM_REDUCERS -Dmapred.map.tasks=$NUM_MAPS -Dmapred.reduce.tasks=$NUM_REDUCERS  /hdfs-fast/input /hdfs-fast/output) 2> >(tee ./logs/HSsort-time-run$i.txt) 
else
    (time spark-submit --class HSSort --deploy-mode ${SPARK_DEPLOY_MODE} --master ${SPARK_MASTER_URL} --conf "spark.driver.memory=${SPARK_DRIVER_MEMORY}" --conf "spark.executor.memory=${SPARK_EXECUTOR_MEMORY}"  --conf "spark.executor.cores=${SPARK_EXECUTOR_CORES}" --conf "spark.executor.instances=${SPARK_EXECUTOR_INSTANCES}" ${HSSORT_JAR}  /hdfs-fast/input /hdfs-fast/output) 2>&1 | (tee ./logs/HSsort-time-run${i}.txt)
fi
result=$?
...
echo "" | tee -a ./TPCx-HS-result-"$prefix".log
./HSDataCheck.sh /hdfs-fast/output | tee -a ./TPCx-HS-result-"$prefix".log
echo "" | tee -a ./TPCx-HS-result-"$prefix".log
...
if [ "$FRAMEWORK" = "MapReduce" ]; then
    (time hadoop jar $HSSORT_JAR HSValidate /hdfs-fast/output  /hdfs-fast/validate) 2> >(tee ./logs/HSvalidate-time-run$i.txt) 
else
    (time spark-submit --class HSValidate --deploy-mode ${SPARK_DEPLOY_MODE} --master ${SPARK_MASTER_URL} --conf "spark.driver.memory=${SPARK_DRIVER_MEMORY}" --conf "spark.executor.memory=${SPARK_EXECUTOR_MEMORY}" --conf "spark.executor.cores=${SPARK_EXECUTOR_CORES}" --conf "spark.executor.instances=${SPARK_EXECUTOR_INSTANCES}" ${HSSORT_JAR} /hdfs-fast/output  /hdfs-fast/validate) 2>&1 | (tee ./logs/HSvalidate-time-run${i}.txt)
fi
result=$?
...
echo "" | tee -a ./TPCx-HS-result-"$prefix".log
./HSDataCheck.sh /hdfs-fast/validate | tee -a ./TPCx-HS-result-"$prefix".log
echo "" | tee -a ./TPCx-HS-result-"$prefix".log
